...
from .fetch import fetch_batch, metas_to_df, BIST_SUBSET
from .io import save_bist
from .db import init_db, ingest_prices, ingest_meta

def parse_args(argv=None):
    p = argparse.ArgumentParser(description="BIST100 Extractor CLI")
    p.add_argument("--range", dest="range_", required=True, help="Yahoo Finance range, e.g., 60d, 1y, 10d")
    p.add_argument("--interval", required=True, help="Interval, e.g., 1d, 30m, 5m, 1m")
    p.add_argument("--db-path", type=Path, default=Path("data/bist100_prices.db"), help="SQLite path (created if missing)")
...
def main(argv=None) -> int:
    ...
    df_all, metas, errs = fetch_batch(
        symbols, rng=args.range_, interval=args.interval,
    ...
    csv_path, xlsx_path = save_bist(df_all, rng=args.range_, interval=args.interval, prefix=args.prefix, out_dir=Path("data"))
    ...
    run_id = ingest_prices(df_all, rng=args.range_, interval=args.interval, db_path=str(args.db_path), note="version 0.1.0")
