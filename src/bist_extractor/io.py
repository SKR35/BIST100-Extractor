...
def save_bist(
    df: pd.DataFrame, rng: str, interval: str, prefix: str = "BIST100", out_dir: Path = Path("data")
):
    """
    ...
    if "datetime" in df_xlsx.columns and pd.api.types.is_datetime64tz_dtype(df_xlsx["datetime"]):
        ...
        df_xlsx["datetime_tr"] = (
            df_xlsx["datetime"].dt.tz_convert("Europe/Istanbul").dt.tz_localize(None)
        )
        ...
        cols = ["datetime_local", "datetime_utc", "datetime_tr", "tz"] + [
            ...
            if c not in ("datetime_local", "datetime_utc", "datetime_tr", "tz")
        ]
